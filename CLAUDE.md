...
```bash
cd ~/cscs-study
hugo server -D --port 1313    # 開發模式，瀏覽器 http://localhost:1313 自動 reload
```
...
1. ❌ **不要 rm -rf `by_book/`**：這是 NSCA 書 24 章內容，已 git tracked，不可丟
2. ❌ **不要改 `/home/hangsau/books/`**：VirtualBox 唯讀
3. ❌ **不要 commit 任何 token / `.env` / `secrets.env`**
4. ❌ **不要主動 `git push`**（per CLAUDE.md §6.1）
